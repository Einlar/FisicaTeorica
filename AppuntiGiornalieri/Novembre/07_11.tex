...
Se invece $\omega=\omega(k)$ non è lineare, si ha \textbf{dispersione}. In tal caso il pacchetto, ossia \q{l'inviluppo} della sovrapposizione delle onde, in genere \textbf{non} viaggia alla stessa velocità delle onde che lo compongono, ma a una certa $v_g$, detta \textbf{velocità di gruppo}, che ora deriviamo.\\
...
\end{align}
Riepilogando: \marginpar{Autofunzioni del potenziale a gradino}
\begin{equation}
\varphi_\mathcal{E}(x) = 
\begin{cases}
...
\]
Notiamo che le $\varphi$ dipendono da $3$ coefficienti, e imponendo le $2$ equazioni di \textit{raccordo} (per continuità e derivabilità) resta un coefficiente non determinato\marginpar{Degenerazione = 1}, e quindi la degenerazione degli autovalori è $1$.\\
Le equazioni di raccordo\marginpar{Condizioni di raccordo} per $\varphi_{\mathcal{E}}$ e $\varphi'_{\mathcal{E}}$ portano a:
\begin{align}
...
Le costanti $c_-^1$ e $c_-^2$ divengono quindi:\marginpar{Costanti in coordinate polari}
\begin{align}
...
\label{eqn:c1-}
\end{align}
Conviene ora \textit{collegare} le costanti alle condizioni iniziali.\marginpar{Condizioni iniziali} In particolare, possiamo scegliere $c^1_+$ (che è un \textit{parametro libero} dato dalla degenerazione degli stati) in modo che il pacchetto sia piccato attorno a $x_0$ all'istante iniziale. In particolare poniamo:
\begin{equation}
c^1_+ = e^{-ikx_0}
\label{eqn:c1+}
\end{equation}
...
\]
Abbiamo $4$ coefficienti e $2$ equazioni di raccordo\marginpar{Degenerazione = 2}, per cui la degenerazione degli autovalori è $4-2=2$.\\
