...
\begin{enumerate}
    \item \textbf{Proiettori}\marginpar{Proiettori} $P\in\mathcal{B}(\mathcal{H})$ è un proiettore se $P=P^\dag$ e $P=P^2$.\\
	...
	\item \textbf{Unitari}:\marginpar{Operatori unitari} Un operatore $U$ \textbf{limitato} $U\in \mathcal{B}(\hs)$ è unitario se soddisfa:
	\begin{equation}
	UU^\dag=U^\dag U=\bb{I}
	\label{eqn:unitario}
	\end{equation}
...
\end{enumerate}
\textbf{Esempio} \marginpar{Esempio: l'operatore momento e la necessità di operatori autoaggiunti}
...
\begin{dfn}
$A$ simmetrico si dice \textbf{autoaggiunto}\marginpar{Operatore autoaggiunto} se $A=A^\dag\Leftrightarrow D\left(A\right)=D\left(A^\dag\right)$ e $\forall\psi\in D(A), A\psi=A^\dag\psi$.
\end{dfn}
...
In particolare in $\dim{\mathcal{H}<\infty}$ ogni $A$ simmetrico è anche hermitiano (autoaggiunto).\\ %Hermitiano = aggiunto (o autoaggiunto)?

Per le matrici hermitiane\marginpar{Proprietà di operatori autoaggiunti} (ossia auto-aggiunte, uguali alla propria trasposta coniugata) sappiamo (teorema spettrale) che possiamo scegliere gli autovettori $\psi_n$ come una base ON di $\hs$. Ciò implica che:
\begin{enumerate}
	...
	\]
	Poiché allora per il teorema spettrale\marginpar{Gli autovalori di $A$ sono tutti e soli quelli "fisici" di $O$} la somma delle probabilità associate agli autovalori è esattamente $1$, si ha che gli autovalori esauriscono tutte le possibilità.\\
	Quindi:
	\begin{align*}
...
\textbf{Riepilogando}:
\begin{itemize}
    \item Le osservabili sono descritte da operatori $A$ lineari (in modo che dipendano dallo stato e non dal vettore usato per rappresentarlo), con dominio $D(A)$ denso in $\hs$ (vogliamo che siano definiti per "tutti" gli stati, e gli altri li vogliamo poter approssimare) %Forse la linearità ha a che fare con l'ambiguità a meno di una fase
