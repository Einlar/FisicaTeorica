...
L'intuizione di Heisenberg fu nel rappresentare un'osservabile in una matrice le cui righe sono ripetizioni \q{shiftate} dei coefficienti della serie di Fourier, in modo che i coefficienti di ordine $0$ riempissero la diagonale principale:
\[
q(t) = \begin{pmatrix}
...
\begin{itemize}
    ...
    \item Come mai sono possibili rappresentazioni degli stessi osservabili come \q{oggetti matematici diversi} (per esempio negli approcci di Heisenberg e Schrödinger, $q_{mn}$ vs $\hat{x}$, $p_{mn}$ vs $-i\hbar \frac{d}{dx}$)
\end{itemize}
...
Notiamo che essendo in generale $\Omega$ non uno spazio vettoriale, non è possibile \q{sommare} stati puri classici.\\
...
Supponiamo $f\geq 0$ (e misurabile, cioè le antimmagini  di intervalli \q{di sopralivello} del codominio, ossia del tipo $f^{-1}(]a,+\infty[)$, sono insiemi misurabili del dominio).\\
...
\begin{enumerate}
	...
	\item Esistono misure di L-S che \q{non pesano i punti} (come nel caso della misura di Dirac), ma per cui non esiste $\alpha^\prime\left(\lambda\right)$ tale che $d\alpha=\alpha^\prime\left(\lambda\right)d\lambda$ ma sono singolarmente continue rispetto alla misura di Lebesgue.
\end{enumerate}
...
Ora mostriamo che, con opportune assunzioni, il valore medio delle osservabili di uno stato definisce un funzionale lineare positivo a cui applicare Riesz-Markov, in modo da associare ad ogni valor medio una \q{struttura probabilistica}.\\
...
\end{equation}
Giustifichiamolo matematicamente. Per il \textbf{teorema di Liouville} per cui in un sistema Hamiltoniano, presa una regione $\Omega$ nello spazio delle fasi $(q,p)$, facendo \q{evolvere} tutti i suoi punti tramite le equazioni di Hamilton per un certo tempo $t$, si giunge ad una nuova regione $\Omega(t)$ che ha lo stesso volume della prima. In altre parole \q{gli stati \textit{si evolvono} come un fluido incompressibile} e vale $dq\,dp=dq\left(t\right)dp(t)$. Perciò, il valor medio di un'osservabile $f$ nello stato $\rho$ al tempo $t$ è dato da:
